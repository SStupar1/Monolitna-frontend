...
export class AdDetailsComponent implements OnInit {
  ...
  private getAvgRating(){
    this.ratingService.getAverageRatingByAd(this.body.id).subscribe(response => {
  ...
  private getComments(){
    this.commentService.getAllCommentsByAd(this.body.id).subscribe(response => {
  ...
  public getDetails(): void {
    this.adService.getAd(this.body.id).subscribe(data =>{
      this.ad = data;
    })
  }

  private setUpAd(): void {
  ...
  addToCart(){
    ...
    this.router.navigateByUrl(`home-page/lists/rent-ad-list`);
  }
  ...
  private publisherExist(ad): boolean{
    ...
    localStorage.setItem('cart', JSON.stringify(this.cart));
    console.log("Evo me ubacen sam u cart");
    console.log(this.cart);
    return retVal;
  }
